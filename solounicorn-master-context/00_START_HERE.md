...
The latest owner instruction overrides two rules in the consolidated reference below: Luck should have **slightly negative long-term expected output**, and **zero cash at quarter close is a loss**. It also explicitly calls for an analog noir garage evolving into an ethereal sci-fi HUD, and valuation driven by ARR, growth and operating stability.

The active playable adapter is documented in `../docs/FOUNDER_RUNTIME.md` and implemented in `../src/game/founder`. Its `founder-candidate.2` profile records numerical overrides and remaining reference-model boundaries. The original quantitative package is preserved as reference evidence; its positive-Luck checks do not validate the active negative-EV distribution.
